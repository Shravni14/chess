...
import { applyMove, getStatus, initialBoard, initialGame, initialState, legalMoves, legalMovesForSquare, positionKey, safeGame, serializeGame, type Board, type GameState } from "../shared/chess";
...
describe("chess rules engine", () => {
  ...
  it("supports castling after the path is clear", () => {
    ...
    const state = initialState();
    const game = { board, state, history: [], captured: { w: [], b: [] }, repetition: { [positionKey(board, state)]: 1 }, lastMove: null };
    expect(legalMovesForSquare(game, [7, 4]).some((move) => move.castle === "K")).toBe(true);
  });
